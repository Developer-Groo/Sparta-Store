...
public class PopularItemService {
    ...
    public List<PopularItemDto> getMostPopularSoldItems(int days) {
        ...
        List<PopularItemRankValueDto> mostPopularSoldItems = soldItemRepository.findMostPopularSoldItems(startDate); // 최근 N일 동안 가장 많이 판매된 상품 목록을 조회.

        return toItemDto(mostPopularSoldItems); // 조회된 데이터를 toItemDto를 통해 PopularItemDto로 변환하여 반환.
    }
