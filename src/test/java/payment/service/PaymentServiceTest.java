...
public class PaymentServiceTest {
    ...
    void checkBeforePayment_notExistsOrder_fail() {
        // given
    ...
    void checkData_success() {
        ...
        // when & then
        assertDoesNotThrow(() -> paymentService.checkData(userId, orderId, amount));
    }

    @Test
    @DisplayName("결제 승인 전, 데이터 검증 실패 - 주문 정보 없음")
    void checkData_notExistsOrder_fail() {
        // given
        Long userId = user.getId();
        String orderId = order.getId();
        long amount = order.getTotalPrice();
        ...
        // when & then
        assertThatThrownBy(() -> paymentService.checkData(userId, orderId, amount))
            .isInstanceOf(CustomException.class)
            .extracting("errorCode")
            .isEqualTo(PaymentErrorCode.NOT_EXISTS_ORDER);
    }
    ...
    @Test
    @DisplayName("결제 취소 시, Payment isCancelled 값 업데이트 및 order 주문상태 변경 성공")
    // org.json.simple의 JSONObject가 제너릭 타입 없이 구현되어 있어서 발생하는 경고무시
    @SuppressWarnings("unchecked")
    void paymentCancelled_success() {
        // given
        JSONObject response = new JSONObject();
        ...
        // then
        then(payment).should(times(1)).updateCancelled();
        then(adminOrderService).should(times(1)).orderCancelled(orderId);

        assertEquals(orderId, payment.getOrder().getId());
    ...
    void paymentCancelled_notExistsPayment_fail() {
        ...
        // when & then
        assertThatThrownBy(() -> paymentService.paymentCancelled(response))
            .isInstanceOf(CustomException.class)
            .extracting("errorCode")
            .isEqualTo(PaymentErrorCode.NOT_EXISTS_PAYMENT);
    }
